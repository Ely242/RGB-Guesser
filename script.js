...
const guessColorLabel = document.getElementById("guess-hex");
...
const modalNewColorButton = document.getElementById("modal-new-color-btn");
const modalTargetHex = document.getElementById("modal-target-hex");
const modalTargetR = document.getElementById("modal-target-r");
...
function getHexCode(values) {
    ...
    const val = (values[0] << 16) | (values[1] << 8) | values[2];
 ...
 */
function channelHeadroom(target) {
...
const rgbSliders = [rSlider, gSlider, bSlider];
document.addEventListener("keydown", (event) => {
    ...
    if (key === "arrowup" || key === "arrowdown" || key === "w" || key === "s") {
        event.preventDefault();
        if (isSliderFocused) {
            const currentIndex = rgbSliders.indexOf(target);
    ...
    if (key === "arrowleft" || key === "arrowright" || key === "a" || key === "d") {
        const slider = (isSliderFocused ? target : rgbSliders[0]);
        if (!slider)
            return;
